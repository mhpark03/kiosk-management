...
- **자동 동기화**: 5분마다 서버와 자동으로 영상 목록을 동기화
...
1. **API URL 설정**: 키오스크 관리 시스템의 API URL을 입력
   - 예: `http://localhost:8080/api`
   - 또는 운영 서버 URL

2. **키오스크 ID 입력**: 이 장치에 할당된 키오스크 ID 입력
   - 키오스크 관리 시스템에서 확인 가능

3. **다운로드 경로 선택**: 영상을 저장할 폴더 선택
   - 기본값: `C:\Users\<사용자>\Downloads\KioskVideos`

4. **자동 동기화 설정**: 5분마다 자동으로 영상 목록을 동기화하려면 체크
...
```json
{
  ...
  "autoSync": true,
  "syncInterval": 5,
  "lastSync": "2025-10-20T10:30:00.000Z"
}
```
...
개발 모드에서 실행하면 자동으로 DevTools가 열립니니다:
