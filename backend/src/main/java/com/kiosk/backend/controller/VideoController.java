...
public class VideoController {
    ...
    /**
     * Add audio to video (Admin only)
     * POST /api/videos/add-audio
     */
    @PostMapping("/add-audio")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<?> addAudioToVideo(
            @RequestBody Map<String, Object> request,
            Authentication authentication) {
        try {
            String userEmail = authentication.getName();
            ...
            Long audioId = Long.valueOf(request.get("audioId").toString());
            String title = (String) request.get("title");
            String description = (String) request.get("description");
            Boolean replaceAudio = request.containsKey("replaceAudio")
                    ? Boolean.valueOf(request.get("replaceAudio").toString())
                    : true;

            log.info("Adding audio to video: videoId={}, audioId={}, replaceAudio={}",
                    videoId, audioId, replaceAudio);

            Video resultVideo = videoService.addAudioToVideo(
                    videoId, audioId, title, description, replaceAudio, user.getId()
            );

            // Record activity
            entityHistoryService.recordVideoActivity(
                    resultVideo.getId(),
                    resultVideo.getTitle(),
                    user,
                    EntityHistory.ActionType.VIDEO_UPLOAD,
                    "비디오에 음성 추가: " + resultVideo.getTitle()
            );

            Map<String, Object> response = new HashMap<>();
            response.put("message", "Audio added to video successfully");
            response.put("video", resultVideo);

            return ResponseEntity.status(HttpStatus.CREATED).body(response);

        } catch (IllegalArgumentException e) {
            log.error("Invalid add-audio request: {}", e.getMessage());
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to add audio to video", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "Failed to add audio to video: " + e.getMessage()));
        }
    }
